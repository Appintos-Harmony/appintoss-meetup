...
- (1) ChatGPT Pro가 누락 요구사항·정책 갭을 발견한 사례: 2026-06-21 4-AI 설계 토의([DEBATE-011](../산출물/09_AI개발파이프라인/토론기록/DEBATE-20260621-011_하모니설계회의.md))에서 ChatGPT Pro가 ① Claude 초안의 "로컬-only 먼저"안이 **핵심 요구(친구 트랙 얹기=크로스유저)를 제거**함을 지적해 비공개 링크 기반 최소 크로스유저를 복원시켰고, ② 전면 다크 테마가 **비게임 심사 위험**(공식 비게임 가이드=라이트)임을 지적해 라이트 TDS로 확정시켰다. Claude가 근거 우위를 인정해 초안을 철회(2라운드)했고 DL-019로 확정. ✅
- (8) 새 앱 재사용 실증: 2026-06-27 하네스 골격(헌장·AGENTS·.claude·.agents·tooling)을 빈 임시 저장소에 복사·`git init` 후 검증 스크립트를 실행 — 비밀값·작업지시서 검사 즉시 PASS, 검증_전체 오케스트레이터가 새 앱 미작성 산출물을 정확히 '누락'으로 보고(게이트 즉시 작동). 자산 11 에이전트·7+3 스킬·4 룰·12 스크립트·2 git훅이 그대로 이식됨. 근거: [재사용 실증 로그](재사용_실증/2026-06-27_하네스_재사용_실증.md). ✅
